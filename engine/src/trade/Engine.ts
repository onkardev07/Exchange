...
export class Engine {
  ...
  createOrder(
    ...
    this.publishWsTickers(fills, market);
  ...
  publishWsTickers(fills: Fill[], market: string) {
    if (fills.length === 0) {
    }
  ...
  updateBalance(
    ...
    if (side === "buy") {
      fills.forEach((fill) => {
        ...
        if (otherUserBalance && userBalance) {
  ...
  checkAndLockFunds(
    ...
    if (userBalance) {
      if (side === "buy") {
        ...
        if ((userBalance[quoteAsset]?.available || 0) < totalCost) {
          throw new Error("Insufficient funds");
        }

        userBalance[quoteAsset].available =
          (userBalance[quoteAsset]?.available || 0) - totalCost;

        userBalance[quoteAsset].locked =
          (userBalance[quoteAsset]?.locked || 0) + totalCost;
      } else {
        ...
        userBalance[baseAsset].available =
          (userBalance[baseAsset]?.available || 0) - Number(quantity);

        userBalance[baseAsset].locked =
          (userBalance[baseAsset]?.locked || 0) + Number(quantity);
      }
    } else {
